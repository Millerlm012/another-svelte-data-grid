<script>
   ...
   */
  let rowStyleFunctions = columns
    .map((col) => col.styleFunc || false)
    .filter((x) => x);

  $: {
    rowStyleFunctions = columns
      .map((col) => col.styleFunc || false)
      .filter((x) => x);
  }
  ...
  let allRows = rows.map((row, i) => {
    //TODO setter probably not needed due to reactive statement
    return {
      i: i,
      data: row,
    };
          ...
          >
            ...
            {:else}
              <div class="cell-default">
                {column.display || column.dataName}
              </div>
            {/if}
          </div>
    ...
    {#each visibleRows as row, i}
      ...
      <div
        ...
        style="top: {getRowTop(row.i, rowHeight)}px; height: {rowHeight}px;
          width: {gridSpaceWidth}px; {style.styleRowBackground(
          row,
          rowStyleFunctions,
          selectedColor,
          selectedTextColor,
