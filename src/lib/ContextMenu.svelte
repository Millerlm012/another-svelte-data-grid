<script>
    ...
    function getTableText(data) {
        let columnNames = columns.map((col) => col.display ? col.display : col.dataName);
    ...
    async function copy(amount) {
        switch (amount) {
            case 'cell':
                await navigator.clipboard.writeText(context.row.data[context.column.dataName]);
                break;
            case 'row':
...
<style>
    ...
    .option:hover { cursor: pointer; background: #0002; }
</style>

<!-- svelte-ignore a11y-click-events-have-key-events -->
<div class="context-menu" transition:fade={{ duration: 100 }} style="top: {context.y + 20}px; left: {context.x + 20}px;">
    <div class="option" on:click={() => copy('cell')}>Copy this cell</div>
    <div class="option" on:click={() => copy('row')}>Copy this row</div>
    {#if selectedRows.length > 0}
        <div class="option" on:click={() => copy('selected')}>Copy selected rows</div>
    {/if}
